...
export function placeholder(seed: string | number, w = 400, h = 300, label = 'ZZU二手市场') {
...
export function campusColor(campus?: string) {
  switch (campus) {
...
export function verifiedBadge(v?: number) {
  if (v === 2) return { text: '已认证', cls: 'bg-brand/10 text-brand' };
  if (v === 1) return { text: '审核中', cls: 'bg-amber-100 text-amber-700' };
