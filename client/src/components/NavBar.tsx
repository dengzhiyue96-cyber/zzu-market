...
export default function NavBar() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-zinc-100 bg-white/95 backdrop-blur">
        ...
        {items.map((it) => (
          <li key={it.to}>
            ...
            >
              ...
              <span className={it.primary ? 'hidden' : ''}>{it.label}</span>
              {it.badge ? (
                <span className="absolute top-1.5 right-[calc(50%-24px)] min-w-[18px] h-[18px] text-[10px] px-1 rounded-full bg-red-500 text-white flex items-center justify-center font-bold">
