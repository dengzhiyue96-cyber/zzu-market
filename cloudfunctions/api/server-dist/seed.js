...
async function main() {
    ...
    for (const c of categories) {
        const existing = await db_1.C.categories().findOne({ name: c.name });
        if (!existing) {
            const id = await (0, db_1.getNextId)('categories');
            await db_1.C.categories().insertOne({ id, ...c });
        }
    }
    const catMap = new Map();
    (await db_1.C.categories().find({}, { projection: { _id: 0 } }).toArray()).forEach((c) => catMap.set(c.name, c.id));
    /* ============ 2. 郑大教材库 ============ */
    const textbooks = [
    ...
    for (let i = 0; i < textbooks.length; i++) {
        const b = textbooks[i];
        const existing = await db_1.C.textbooks().findOne({ book_name: b.book_name });
        if (!existing) {
            const id = await (0, db_1.getNextId)('textbooks');
            await db_1.C.textbooks().insertOne({ id, ...b, sort: i + 1 });
        }
    }
    const tbMap = new Map();
    (await db_1.C.textbooks().find({}, { projection: { _id: 0 } }).toArray()).forEach((t) => tbMap.set(t.book_name, t.id));
    /* ============ 3. 测试用户 ============ */
    const users = [
    ...
    for (const u of users) {
        const existing = await db_1.C.users().findOne({ username: u.username });
        if (!existing) {
            const id = await (0, db_1.getNextId)('users');
            await db_1.C.users().insertOne({ id, ...u, password: hash, avatar: null, student_id: null, verify_code: '', created_at: now, updated_at: now });
        }
    }
    const userMap = new Map();
    (await db_1.C.users().find({}, { projection: { _id: 0 } }).toArray()).forEach((u) => userMap.set(u.username, u.id));
    /* ============ 4. 测试商品 ============ */
    ...
    let prodCount = await db_1.C.products().countDocuments();
    if (prodCount === 0) {
    ...
    let wantedCount = await db_1.C.wanted().countDocuments();
    if (wantedCount === 0) {
